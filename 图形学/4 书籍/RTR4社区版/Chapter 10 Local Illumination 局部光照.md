...
![图10.11：常用面光源的形状。从左到右依次：球形、矩形（卡牌光源）、管状（线光源）、管状的聚焦发射光源（辐射能量在半球上的分布不均匀，集中在光源的法线方向上）。请注意它们所生成的、不同形状的高光。](202306161234847.png "图10.11：常用面光源的形状。从左到右依次：球形、矩形（卡牌光源）、管状（线光源）、管状的聚焦发射光源（辐射能量在半球上的分布不均匀，集中在光源的法线方向上）。请注意它们所生成的、不同形状的高光。")
...
![图10.23：反射映射。当相机看到一个物体的时候，使用表面法线 \mathbf{n} 与观察向量 \mathbf{v} 来计算反射观察向量 \mathbf{r} ，并使用这个反射观察向量 \mathbf{r} 来访问环境贴图。通过使用一些投影函数，来将反射观察向量 (x, y, z) 转换为纹理坐标，从而检索环境贴图所存储的radiance。](image_eghlil4rO1.png "图10.23：反射映射。当相机看到一个物体的时候，使用表面法线 \mathbf{n} 与观察向量 \mathbf{v} 来计算反射观察向量 \mathbf{r} ，并使用这个反射观察向量 \mathbf{r} 来访问环境贴图。通过使用一些投影函数，来将反射观察向量 (x, y, z) 转换为纹理坐标，从而检索环境贴图所存储的radiance。")
...
![图10.24：一个位于相机处的光源，以及凹凸映射与环境映射相结合的渲染效果。从左到右分别是：没有环境映射、没有凹凸映射、相机处没有光源、所有这三个特性组合在一起。](202306192119111.png "图10.24：一个位于相机处的光源，以及凹凸映射与环境映射相结合的渲染效果。从左到右分别是：没有环境映射、没有凹凸映射、相机处没有光源、所有这三个特性组合在一起。")
...
![图10.25：地球上的经纬度线是等距的，这与与传统的Mercator投影不同。](202306201032310.png "图10.25：地球上的经纬度线是等距的，这与与传统的Mercator投影不同。")
...
![图10.26：球面贴图（左）和 经纬度映射 格式的等效贴图（右）。](202306201040078.png "图10.26：球面贴图（左）和 经纬度映射 格式的等效贴图（右）。")
...
![图10.27：在给定的球面贴图空间中，原始观察向量 \mathbf{v} 是恒定的，球面贴图的法线 \mathbf{n} 介于原始观察向量 \mathbf{v} 与反射观察向量 \mathbf{r} 之间。对于位于原点的单位球体而言，图中的交点 \mathbf{h} 与单位法线 \mathbf{n} 具有相同的坐标。同时，图中还展示了为什么 h\_y （从原点开始测量）与球体贴图的纹理坐标 v （不要与观察向量 \mathbf{v} 混淆）有关。](202306201106396.png "图10.27：在给定的球面贴图空间中，原始观察向量 \mathbf{v} 是恒定的，球面贴图的法线 \mathbf{n} 介于原始观察向量 \mathbf{v} 与反射观察向量 \mathbf{r} 之间。对于位于原点的单位球体而言，图中的交点 \mathbf{h} 与单位法线 \mathbf{n} 具有相同的坐标。同时，图中还展示了为什么 h_y （从原点开始测量）与球体贴图的纹理坐标 v （不要与观察向量 \mathbf{v} 混淆）有关。")
...
![图10.28：MatCap渲染的例子。左侧物体使用了右侧两个球面贴图进行着色处理。其中右上方的贴图使用了观察空间中的法线来进行索引；而右下角的贴图则使用了观察空间中的反射向量来进行索引，并将二者的值相加。最终生成的效果是相当令人信服的，但是移动观察点，图像中的光照效果并不会发生改变。](202306201141333.png "图10.28：MatCap渲染的例子。左侧物体使用了右侧两个球面贴图进行着色处理。其中右上方的贴图使用了观察空间中的法线来进行索引；而右下角的贴图则使用了观察空间中的反射向量来进行索引，并将二者的值相加。最终生成的效果是相当令人信服的，但是移动观察点，图像中的光照效果并不会发生改变。")
...
![图10.29：Greene的环境贴图。图中展示了对应的关键点，左侧的立方体会被展开到右侧的环境贴图中。](202306201542442.png "图10.29：Greene的环境贴图。图中展示了对应的关键点，左侧的立方体会被展开到右侧的环境贴图中。")

![图10.30：与 图10.26 中所使用的环境贴图相同，不同之处在于这里将其转换为了立方体贴图的格式。](202306201543508.png "图10.30：与 图10.26 中所使用的环境贴图相同，不同之处在于这里将其转换为了立方体贴图的格式。")
...
![图10.31：《极限竞速7》中环境贴图的光照，会随着赛车位置的改变而更新。](202306201605223.png "图10.31：《极限竞速7》中环境贴图的光照，会随着赛车位置的改变而更新。")
...
![图10.32：球体的立方体展开和八面体展开。](202306201634036.png "图10.32：球体的立方体展开和八面体展开。")
...
![图10.33：在第一行中，左侧是原始环境贴图，右侧是将其应用到一个球体上的着色结果。在第二行中，使用高斯滤波器对相同的环境贴图进行模糊处理，模拟了粗糙材质的外观。](202306201736675.png "图10.33：在第一行中，左侧是原始环境贴图，右侧是将其应用到一个球体上的着色结果。在第二行中，使用高斯滤波器对相同的环境贴图进行模糊处理，模拟了粗糙材质的外观。")
...
![图10.34：左侧展示了一个被物体反射的光线，它被反射到了完美镜面反射方向上，使用这个方向来从环境纹理（在本例中是立方体贴图）中检索对应的值。右图展示了反射光线的镜面波瓣，它用于对环境纹理进行采样。绿色方块表示立方体贴图的横截面，红色的虚线代表了纹素之间的边界。](202306201747328.png "图10.34：左侧展示了一个被物体反射的光线，它被反射到了完美镜面反射方向上，使用这个方向来从环境纹理（在本例中是立方体贴图）中检索对应的值。右图展示了反射光线的镜面波瓣，它用于对环境纹理进行采样。绿色方块表示立方体贴图的横截面，红色的虚线代表了纹素之间的边界。")
...
![图10.35：两个观察者看向同一个闪亮的球体。球体上两个不同的位置对于两个观察者产生了相同的反射方向。左侧观察者所看到的表面反射是一个对称的波瓣。右边观察者所看到的反射波瓣则会被表面本身的视界所截断，因为光线并不会被视界（地平线）以下的表面所反射。](202306201853156.png "图10.35：两个观察者看向同一个闪亮的球体。球体上两个不同的位置对于两个观察者产生了相同的反射方向。左侧观察者所看到的表面反射是一个对称的波瓣。右边观察者所看到的反射波瓣则会被表面本身的视界所截断，因为光线并不会被视界（地平线）以下的表面所反射。")
...
![图10.36：环境贴图预过滤。将立方体贴图与不同粗糙度的GGX波瓣进行卷积，并将结果存储在纹理的mipmap中。粗糙度从左到右递减，下面一行展示了生成的对应mipmap，上面一行展示了在反射方向上渲染的球体。](202306201900507.png "图10.36：环境贴图预过滤。将立方体贴图与不同粗糙度的GGX波瓣进行卷积，并将结果存储在纹理的mipmap中。粗糙度从左到右递减，下面一行展示了生成的对应mipmap，上面一行展示了在反射方向上渲染的球体。")
...
![图10.37：Karis的“分裂求和”近似。从左到右，材质的粗糙度越来越高。第一行：参考解决方案。第二行：分裂积分近似。第三行：在镜面波瓣上加入所需径向对称（ \mathbf{n}=\mathbf{v}=\mathbf{r} ）的分裂积分。最后一行会引入最多的误差。](202306211040046.png "图10.37：Karis的“分裂求和”近似。从左到右，材质的粗糙度越来越高。第一行：参考解决方案。第二行：分裂积分近似。第三行：在镜面波瓣上加入所需径向对称（ \mathbf{n}=\mathbf{v}=\mathbf{r} ）的分裂积分。最后一行会引入最多的误差。")
...
![图10.38：红色代表了GGX BRDF，绿色代表了围绕反射向量径向对称的GGX NDF波瓣。后者进行了缩放修正，从而与GGX镜面波瓣的峰值相匹配，但是请注意，它无法捕获基于半向量BRDF的各向异性形状。在最右边，使用这两个波瓣各自渲染了一个球体，请注意二者之间高光的差异。](202306211104438.png "图10.38：红色代表了GGX BRDF，绿色代表了围绕反射向量径向对称的GGX NDF波瓣。后者进行了缩放修正，从而与GGX镜面波瓣的峰值相匹配，但是请注意，它无法捕获基于半向量BRDF的各向异性形状。在最右边，使用这两个波瓣各自渲染了一个球体，请注意二者之间高光的差异。")
...
![图10.39：计算irradiance环境贴图的过程。在原始环境纹理（本例中是一个立方体贴图）中对表面法线周围的余弦加权半球进行采样，并对采样结果进行求和，从而获得与视图无关的irradiance。图中绿色的方块代表了立方体贴图的横截面，红色的虚线代表了纹素之间的边界。这里使用的是一个立方体贴图，但实际上任何环境表示都是可以使用的。](202306211248385.png "图10.39：计算irradiance环境贴图的过程。在原始环境纹理（本例中是一个立方体贴图）中对表面法线周围的余弦加权半球进行采样，并对采样结果进行求和，从而获得与视图无关的irradiance。图中绿色的方块代表了立方体贴图的横截面，红色的虚线代表了纹素之间的边界。这里使用的是一个立方体贴图，但实际上任何环境表示都是可以使用的。")
...
![图10.40：从Grace大教堂球面贴图生成的irradiance贴图。左侧图像是原始的球面贴图。右侧图像则是通过将每个像素上半球范围内的颜色进行加权求和而得到的。](202306211312886.png "图10.40：从Grace大教堂球面贴图生成的irradiance贴图。左侧图像是原始的球面贴图。右侧图像则是通过将每个像素上半球范围内的颜色进行加权求和而得到的。")

![图10.41：使用irradiance贴图生成的人物光照，来自《死或生3》。](202306211315186.png "图10.41：使用irradiance贴图生成的人物光照，来自《死或生3》。")
...
![图10.42：立方体贴图（左）及其过滤后的irradiance贴图（右）。](202306211316823.png "图10.42：立方体贴图（左）及其过滤后的irradiance贴图（右）。")
...
![图10.43：clamped余弦函数（红色）与包含九个系数的球谐函数近似（蓝色）。近似结果十分接近。请注意在 π/2 到 π 之间，蓝色曲线有轻微的下降和上升。](202306211658563.png "图10.43：clamped余弦函数（红色）与包含九个系数的球谐函数近似（蓝色）。近似结果十分接近。请注意在 π/2 到 π 之间，蓝色曲线有轻微的下降和上升。")
...
![图10.44：左图：由振铃现象所引起的视觉瑕疵。右图：一种可能的解决方案是让原始函数变得更加平滑，这样它就可以避免振铃现象，这个过程被称为“窗口化（windowing）”。](202306211701378.png "图10.44：左图：由振铃现象所引起的视觉瑕疵。右图：一种可能的解决方案是让原始函数变得更加平滑，这样它就可以避免振铃现象，这个过程被称为“窗口化（windowing）”。")
...
![图10.45：irradiance编码的各种方式。从左到右分别是：使用环境贴图和对irradiance进行蒙特卡罗积分计算的漫反射光照；使用环境立方体（ambient cube）编码的irradiance；球谐函数；球面高斯函数；H-basis（它只能表示一个半球方向，因此指向下方的法线不会被着色)。](202306212029467.png "图10.45：irradiance编码的各种方式。从左到右分别是：使用环境贴图和对irradiance进行蒙特卡罗积分计算的漫反射光照；使用环境立方体（ambient cube）编码的irradiance；球谐函数；球面高斯函数；H-basis（它只能表示一个半球方向，因此指向下方的法线不会被着色)。")
...
![图10.46：电影行业中的人工照明。](202306212146394.png "图10.46：电影行业中的人工照明。")
...
![图10.47：相同的圆盘光源有着两种不同的发射方式。左边：圆盘上的每一点都均匀地向外半球发射光线。右边：radiance集中在圆盘法线周围的波瓣上。](202306212147521.png "图10.47：相同的圆盘光源有着两种不同的发射方式。左边：圆盘上的每一点都均匀地向外半球发射光线。右边：radiance集中在圆盘法线周围的波瓣上。")
