...
3. $$V=|\mathbf{a}\cdot (\mathbf{b}\times\mathbf{c})|=|\mathbf{b}\cdot (\mathbf{c}\times\mathbf{a})|=|\mathbf{c}\cdot (\mathbf{a}\times\mathbf{b})|$$


向量 $S_1,AB,S_2$ 是所组成的平行六面体的三条棱，我们要求的距离是垂线 d
即
$$|\mathbf{S_1}\cdot (\mathbf{S_2}\times\mathbf{AB})|=|S_{1}\times S_2|h$$
这样就可以求出h
