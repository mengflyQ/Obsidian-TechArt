...
4. 将 Z 分量加 1 减 2 重新映射回法线贴图的 B 通道中。

## 8. 什么是 ao？游戏流程中有哪些实现 ao 的方式？​


AO（Ambient Occlusion）是描述均匀环境光下每个位置能接收到光线的强度合集，是一种为了增加渲染真实感的方法。​
...
1. 在计算机图形编程中，drawcall（绘制调用）是指 CPU 向 GPU 发出的命令，用于渲染或绘制一个或多个图形对象。每个 drawcall 命令都包含了绘制一个或多个三维图形对象所需的所有信息，例如对象的顶点数据、纹理贴图、着色器程序等。​
...
延迟渲染是将**着色（Shading）** 延迟到深度计算之后进行处理的一种渲染方法。首先会将场景中所有物体的片元信息通过 MRT 技术绘制到 GBuffer 中，最后根据 gbuffer 中的信息进行逐像素光照计算。

延迟渲染管线和前向渲染管线的差异如下​
- 后处理方式不同：前向渲染需要额外渲染深度图，而延迟渲染可以直接使用已有的深度图。​
- 着色计算不同：前向渲染可以对不同物体使用不同的渲染方式，​延迟渲染因为是最后统一计算光照的，所以只能用一个光照模型（如果需要其他光照模型，只能切换 pass）
- 抗锯齿不同：延迟渲染对 MSAA 算法不友好，无法对已降采样的片元增加采样点实现抗锯齿。
...
## Early-Z
**在光栅化阶段（三角形遍历阶段）之后、像素处理阶段之前**，加入一个 Early-Z 阶段，进行深度测试。
减少了 overdraw
## Pre-Z
...
- Unity 的 UV 是 OpenGL 坐标系下的，uv 空间落在第一象限（左下角为原点）。UE4 的 UV 是 DirectX 坐标系下的，uv 空间落在第四象限（左上角为原点）。​
...
解决办法：超采样（把一个像素细分为更多个小的采样点进行采样）

超采样消耗过大，现在我们通常采用 mipmap（多级渐远纹理）
...
**GPU 中的计算方法**：在屏幕空间中计算出当前像素点与右方像素点和上方像素点在 Texture space 的距离，二者取最大值，再取对数即可得到 D (D 表示 Level 等级)
...
**产生 Overblur 的原因是因为，所采用的不同 level 的 Mipmap 默认的都是正方形区域的 Range Query**。但事实情况如图：有各种不规则图形
![[1684499675236.png]]
...
而点光源方向是向球形的，做法和传统的 shadowmap 有区别，采用了 Omnidirectional Shadow Maps（全方向的阴影贴图）技术，使用了 cubemap 纹理。
...
Shader "Custom/SimpleColor"
{
    ...
    SubShader
    {
        ...
        //6. BUFFER_START 和 CBUFFER_END 宏定义用于创建一个常量缓冲区块，并指定其中包含的数据成员
...
由于这个原因，我们将不会使用球体的采样核心，而使用一个**沿着表面法向量的半球体**采样核心。这要求我们生成法线图，或者从深度图从重建法线。
